...
Elm.Model.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var clearFlash = F2(function (id,
   model) {
      return _U.replace([["flashMessages"
                         ,A2($List.filter,
                         function (msg) {
                            return !_U.eq(msg.id,id);
                         },
                         model.flashMessages)]],
      model);
   });
   var addFlash = F2(function (text,
   model) {
      return function () {
         var newFlash = {_: {}
                        ...
                        ,text: text};
         return {ctor: "_Tuple2"
                ,_0: _U.replace([["latestFlashId"
                                 ,model.latestFlashId + 1]
                                ,["flashMessages"
                                 ,A2($List._op["::"],
                                 newFlash,
                                 model.flashMessages)]],
                model)
                ,_1: model.latestFlashId};
      }();
   });
   var isDirty = function (model) {
      return function () {
         var _v0 = model.lastSaved;
         switch (_v0.ctor)
         {case "Just":
            return !_U.eq(_v0._0,
              model.todos.currentState);
            case "Nothing":
            return !_U.eq(model.todos.currentState,
              _L.fromArray([]));}
         _U.badCase($moduleName,
         "between lines 165 and 170");
      }();
   };
   var getOrCrash = F2(function (msg,
   maybe) {
      return function () {
         switch (maybe.ctor)
         {case "Just": return maybe._0;
            case "Nothing":
            return $Debug.crash(msg);}
         _U.badCase($moduleName,
         "between lines 154 and 159");
      }();
   });
   var canRedo = function (undoTodoList) {
      return $Basics.not($List.isEmpty(undoTodoList.futureStates));
   };
   var canUndo = function (undoTodoList) {
      return $Basics.not($List.isEmpty(undoTodoList.pastStates));
   };
   var redo = function (undoTodoList) {
      return function () {
         var _v4 = undoTodoList.futureStates;
         switch (_v4.ctor)
         {case "::":
            return $Maybe.Just({_: {}
                               ,currentState: _v4._0
                               ,futureStates: _v4._1
                               ,pastStates: A2($List._op["::"],
                               undoTodoList.currentState,
                               undoTodoList.pastStates)});}
         return $Maybe.Nothing;
      }();
   };
   var undo = function (undoTodoList) {
      return function () {
         var _v7 = undoTodoList.pastStates;
         switch (_v7.ctor)
         {case "::":
            return $Maybe.Just({_: {}
                               ,currentState: _v7._0
                               ,futureStates: A2($List._op["::"],
                               undoTodoList.currentState,
                               undoTodoList.futureStates)
                               ,pastStates: _v7._1});}
         return $Maybe.Nothing;
      }();
   };
   var updateList = F2(function (updateFun,
   undoTodoList) {
      return function () {
         var newList = updateFun(undoTodoList.currentState);
         return {_: {}
                ,currentState: newList
                ,futureStates: _L.fromArray([])
                ,pastStates: A2($List._op["::"],
                undoTodoList.currentState,
                undoTodoList.pastStates)};
      }();
   });
   var add = F2(function (todo,
   todoList) {
      return A2($List._op["::"],
      todo,
      todoList);
   });
   var $delete = F2(function (id,
   todos) {
      return A2($List.filter,
      function (todo) {
         return !_U.eq(todo.id,id);
      },
      todos);
   });
   var markDone = F3(function (id,
   done,
   todos) {
      return function () {
         var update = function (todo) {
            return _U.eq(todo.id,
            id) ? _U.replace([["done"
                              ,done]],
            todo) : todo;
         };
   ...
   var initialUndoTodoList = function (initialState) {
      return {_: {}
             ,currentState: initialState
             ,futureStates: _L.fromArray([])
             ,pastStates: _L.fromArray([])};
   };
   var UndoTodoList = F3(function (a,
   b,
   c) {
      return {_: {}
             ,currentState: b
   ...
   var Chillin = {ctor: "Chillin"};
   var initialModel = function (initialTodos) {
      return {_: {}
             ,currentText: ""
             ,flashMessages: _L.fromArray([])
             ,lastSaved: $Maybe.Nothing
             ,latestFlashId: 0
             ,latestId: $List.length(initialTodos)
             ,saveState: Chillin
             ,todos: initialUndoTodoList(A2($List.map2,
             F2(function (id,todo) {
                return {_: {}
                       ,done: todo.done
                       ,id: id
             ...
             _L.range(0,
             $List.length(initialTodos)))(initialTodos))};
   };
   var Saving = function (a) {
      return {ctor: "Saving"
             ,_0: a};
   };
   var FlashMessage = F2(function (a,
   b) {
      return {_: {},id: a,text: b};
   });
   ...
   _elm.Model.values = {_op: _op
                       ...
                       ,Saving: Saving
                       ,Chillin: Chillin
                       ,UndoTodoList: UndoTodoList
                       ,initialUndoTodoList: initialUndoTodoList
                       ,Todo: Todo
...
Elm.Save = Elm.Save || {};
Elm.Save.make = function (_elm) {
   "use strict";
   _elm.Save = _elm.Save || {};
   if (_elm.Save.values)
   return _elm.Save.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var encodeTodo = function (todo) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "text"
                                               ,_1: $Json$Encode.string(todo.text)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "done"
                                               ,_1: $Json$Encode.bool(todo.done)}]));
   };
   var encodeTodoList = function (todos) {
      return $Json$Encode.list(A2($List.map,
      encodeTodo,
      todos));
   };
   var encode = function (todos) {
      return A2($Json$Encode.encode,
      4,
      encodeTodoList(todos));
   };
   var save = function (todos) {
      return $Task.map($Basics.always({ctor: "_Tuple0"}))($Task.mapError($Basics.toString)(A2($Http.send,
   ...
   _elm.Save.values = {_op: _op
                      ,encode: encode
                      ,encodeTodoList: encodeTodoList
...
Elm.Todos.make = function (_elm) {
   ...
   $Model = Elm.Model.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Save = Elm.Save.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartApp = Elm.StartApp.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Time = Elm.Time.make(_elm);
   var initialTodos = Elm.Native.Port.make(_elm).inbound("initialTodos",
   "List { text : String, done : Bool }",
   function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "text" in v && "done" in v ? {_: {}
                                                                      ...
                                                                      v.done)} : _U.badPort("an object with fields `text`, `done`",
         v);
      })) : _U.badPort("an array",v);
   });
   var NoOp = {ctor: "NoOp"};
   var ClearFlashMessage = function (a) {
      return {ctor: "ClearFlashMessage"
             ,_0: a};
   };
   var clearAfterTimeout = function (id) {
      return A2($Task.andThen,
      $Task.sleep(2 * $Time.second),
      function (_v0) {
         return function () {
            return $Task.succeed(ClearFlashMessage(id));
         }();
      });
   };
   var SaveSucceeded = {ctor: "SaveSucceeded"};
   var SaveFailed = {ctor: "SaveFailed"};
   ...
   model) {
      return function () {
         ...
         {case "AddTodo":
            ...
            return {ctor: "_Tuple2"
                   ,_0: _U.replace([["todos"
                                    ,$Model.getOrCrash("couldn\'t redo")($Model.redo(model.todos))]],
                   model)
                   ,_1: $Effects.none};
            case "Save":
            return {ctor: "_Tuple2"
                   ,_0: _U.replace([["saveState"
                                    ,$Model.Saving(model.todos.currentState)]],
                   model)
                   ,_1: $Effects.task($Task.map(function (result) {
                      return function () {
                         var _v8 = A2($Debug.log,
                         "result",
                         result);
                         switch (_v8.ctor)
                         {case "Err": return SaveFailed;
                            case "Ok":
                            return SaveSucceeded;}
                         _U.badCase($moduleName,
                         "between lines 91 and 97");
                      }();
                   })($Task.toResult($Save.save(model.todos.currentState))))};
            case "SaveFailed":
            return function () {
                 var _v11 = model.saveState;
                 switch (_v11.ctor)
                 {case "Saving":
                    return function () {
                         var $ = A2($Model.addFlash,
                         "Save failed",
                         model),
                         newModel = $._0,
                         flashId = $._1;
                         return {ctor: "_Tuple2"
                                ,_0: _U.replace([["saveState"
                                                 ,$Model.Chillin]],
                                newModel)
                                ,_1: $Effects.task(clearAfterTimeout(flashId))};
                      }();}
                 return $Debug.crash(A2($Basics._op["++"],
                 "SaveFailed in state ",
                 $Basics.toString(model.saveState)));
              }();
            case "SaveSucceeded":
            return function () {
                 var _v13 = model.saveState;
                 switch (_v13.ctor)
                 {case "Saving":
                    return function () {
                         var $ = A2($Model.addFlash,
                 ...
                 return $Debug.crash(A2($Basics._op["++"],
                 "SaveSucceeded in state ",
                 $Basics.toString(model.saveState)));
              }();
            case "Undo":
            return {ctor: "_Tuple2"
                   ,_0: _U.replace([["todos"
                                    ,$Model.getOrCrash("couldn\'t undo")($Model.undo(model.todos))]],
                   model)
                   ,_1: $Effects.none};
            case "UpdateText":
            return {ctor: "_Tuple2"
   ...
   var MarkDone = F2(function (a,
   b) {
      return {ctor: "MarkDone"
             ,_0: a
             ,_1: b};
   });
   var viewTodo = F2(function (addr,
   todo) {
      return A2($Html.li,
      _L.fromArray([]),
      A2($Basics._op["++"],
      _L.fromArray([A2($Html.input,
                   _L.fromArray([$Html$Attributes.type$("checkbox")
                                ,$Html$Attributes.checked(todo.done)
                                ,A3($Html$Events.on,
                                "change",
                                ...
                                function (checked) {
                                   return A2($Signal.message,
                                   addr,
                                   A2(MarkDone,todo.id,checked));
                                })]),
                   _L.fromArray([]))
                   ,A2($Html.span,
                   _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                      ,_0: "text-decoration"
                                                                      ,_1: todo.done ? "line-through" : "none"}]))]),
                   _L.fromArray([$Html.text(todo.text)]))]),
      todo.done ? _L.fromArray([$Html.text(" ")
                               ,A2($Html.a,
                               _L.fromArray([A2($Html$Events.onClick,
                               addr,
                               Delete(todo.id))]),
                               _L.fromArray([$Html.text("delete")]))]) : _L.fromArray([])));
   });
      ...
      _L.fromArray([A2($Html.input,
                   ...
                   _L.fromArray([A2($Html.button,
                                _L.fromArray([A2($Html$Events.onClick,
                                             addr,
                                             Undo)
                                             ,$Html$Attributes.disabled($Basics.not($Model.canUndo(model.todos)))]),
                                _L.fromArray([$Html.text("Undo")]))
                                ,A2($Html.button,
                                _L.fromArray([A2($Html$Events.onClick,
                                             addr,
                                             Redo)
                                             ,$Html$Attributes.disabled($Basics.not($Model.canRedo(model.todos)))]),
                                _L.fromArray([$Html.text("Redo")]))
                                ,A2($Html.button,
                                _L.fromArray([A2($Html$Events.onClick,
                                             addr,
                                             Save)
                                             ,$Html$Attributes.disabled($Basics.not($Model.isDirty(model)))]),
                   ...
                   function (msg) {
                      return A2($Html.li,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text(msg.text)]));
                   },
                   model.flashMessages))]));
   });
   var config = {_: {}
                ,init: {ctor: "_Tuple2"
                       ,_0: $Model.initialModel(initialTodos)
                       ,_1: $Effects.none}
                ,inputs: _L.fromArray([])
                ,update: update
                ,view: view};
